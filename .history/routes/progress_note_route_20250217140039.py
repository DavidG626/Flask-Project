...
def edit_progress_note(note_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if exam_date:
                progress_note.date_of_exam = datetime.strptime(exam_date, '%m/%d/%Y')

            injury_date = request.form.get('date_of_injury')
            if injury_date:
                progress_note.date_of_injury = datetime.strptime(injury_date, '%m/%d/%Y')

            rtw_date = request.form.get('return_to_work')
            if rtw_date:
                progress_note.return_to_work = datetime.strptime(rtw_date, '%m/%d/%Y')
            ...
            return redirect(url_for('pr2.edit_progress_note', note_id=note_id))

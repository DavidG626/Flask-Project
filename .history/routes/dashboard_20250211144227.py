...
def patient_detail(patient_id):
    ...
    # Query all ICD10 codes (or filter based on your needs)
    icd10_codes = ICD10Code.query.all()

    return render_template('patient_info.html',
                         patient=patient,
                         provider=current_user,
                         patient_diagnoses=patient.diagnoses,  # Assuming a relationship
...
def patient_medical_history(patient_id):
    ...
    # Get the diagnoses with their ICD10 codes
    diagnoses = []
    for diagnosis in patient.diagnoses:
        icd10_code = ICD10Code.query.get(diagnosis.icd10_codes)  # Changed from icd10_id
        if icd10_code:
            diagnoses.append(icd10_code)

...
def update_medical_history(patient_id):
    ...
    if request.method == 'POST':
        try:
            # Ensure a medical history object exists for this patient
            if not patient.medical_history:
                medical_history = PastMedicalHistory(patient_id=patient.id)
                db.session.add(medical_history)
                db.session.commit()  # Commit immediately to get the medical_history.id
            else:
                medical_history = patient.medical_history

            # Update medical conditions (with ICD10 codes)
            existing_conditions = {condition.id: condition for condition in medical_history.medical_conditions}

            for i in range(1, 11):
                condition_name = request.form.get(f'condition_{i}')
                icd10_code = request.form.get(f'icd10_code_{i}')  # Assuming you have a form field for ICD10 codes

                if condition_name and condition_name.strip():
                    # Check if the condition already exists (modify if necessary)
                    condition = next((c for c in existing_conditions.values() if c.condition_name == condition_name.strip()), None)

                    if condition:
                        # Update existing condition's ICD10 code if provided
                        if icd10_code:
                            condition.icd10_code_id = int(icd10_code)
                    else:
                        ...
                        new_condition = MedicalCondition(
                            ...
                            icd10_code_id=int(icd10_code) if icd10_code else None
                        )
            ...
            return redirect(url_for('dashboard.medical_history', patient_id=patient_id))

        except Exception as e:
            db.session.rollback()
            flash(f'An error occurred while updating medical history: {str(e)}', 'error')
            return redirect(url_for('dashboard.medical_history', patient_id=patient_id))

    # For GET request, render the page (added return here)

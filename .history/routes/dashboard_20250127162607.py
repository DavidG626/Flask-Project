...
def update_patient(patient_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Update lawyer info
            # Update lawyer info
        if patient.lawyer:  # If patient has a lawyer record
            if request.form.get('lawyer_name'):  # If new lawyer info provided
                # Update lawyer info
            ...
            else:  # If lawyer fields are empty, delete lawyer record
                db.session.delete(patient.lawyer)
        elif request.form.get('lawyer_name'):  # If no existing lawyer but new info provided
            # Create new lawyer
            new_lawyer = Lawyer(
                patient_id=patient.id,

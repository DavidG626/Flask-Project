...
from models.wc_patient_models import Patient, BodyPart, Employer, ClaimsAdmin, Lawyer, Provider  # Import Provider model
from datetime import datetime

# Create the login_register blueprint
wc_patient_portal_bp = Blueprint('wc_patient_portal', __name__)

# Define routes for patient portal
@wc_patient_portal_bp.route("/wc_patient_portal_register", methods=["GET", "POST"])
def register():
    form = wc_PatientRegistrationForm()
    if form.validate_on_submit():
        try:
            # Look up the provider based on first and last name
            provider = Provider.query.filter_by(
                first_name=form.provider_first_name.data,
                last_name=form.provider_last_name.data
            ).first()

            if not provider:
                flash('Provider not found. Please check the details and try again.', 'danger')
                return render_template('wc_patient_portal.html', title="Register", form=form)

            # Create a new patient linked to the provider
            patient = Patient(
                ...
                provider_id=provider.id  # Link the patient to the provider securely
            )

...
from models.patient_medical_hx_models import Allergy, PastMedicalHistory, MedicalCondition, Surgery, Medication, LabResult, ImagingResult, OperativeReport, Auth, Misc
...
# Start of Auths
@dashboard_bp.route('/patient/<int:patient_id>/authorizations')
@login_required
@check_session_timeout
...
# Start Misc
@dashboard_bp.route('/patient/<int:patient_id>/misc')
@login_required
@check_session_timeout
def misc(patient_id):
    patient_misc = Patient.query.get_or_404(patient_id)

    # Verify this patient belongs to the current provider
    if (patient_misc.provider_first_name != current_user.first_name or 
        patient_misc.provider_last_name != current_user.last_name):
        flash('You do not have permission to view this patient.', 'error')
        return redirect(url_for('dashboard.my_patients'))
    
    return render_template('misc.html', patient_id=patient_id)


@dashboard_bp.route('/upload_misc/<int:patient_id>', methods=['GET', 'POST'])
@login_required
@check_session_timeout
...
@dashboard_bp.route('/view_misc/<int:misc_id>')
@login_required
@check_session_timeout
...
@dashboard_bp.route('/download_misc/<int:misc_id>')
@login_required
@check_session_timeout
...
@dashboard_bp.route('/delete_misc/<int:misc_id>', methods=['POST'])
@login_required
@check_session_timeout

...
def upload_operative_report(patient_id):
    ...
    if (patient.provider_first_name != current_user.first_name or 
        patient.provider_last_name != current_user.last_name):
        flash('You do not have permission to upload operative reports for this patient.', 'error')
        return redirect(url_for('dashboard.my_patients'))
    
    if request.method == 'POST':
        # Check if any file was uploaded
        if 'operative_file' not in request.files:  # Changed to match form input name
            flash('No file selected', 'error')
            return redirect(request.url)
        ...
        if file:
            try:  # Added error handling
                ...
                # Create operative report record
                ...
                db.session.add(operative_report)
                db.session.commit()
                ...
                print(f"Error saving operative report: {str(e)}")  # Added debug print
                db.session.rollback()
                flash('Error saving operative report', 'error')
                return redirect(request.url)

    ...
    if (patient.provider_first_name != current_user.first_name or 
        ...
        if request.method == 'POST':
            try:
                ...
                if request.method == 'POST':
            try:
                # Your existing code for updating progress note fields and checkboxes...

                # Handle diagnoses update
                # First, remove existing diagnoses if a 'remove_diagnosis' was clicked
                diagnosis_to_remove = request.form.get('remove_diagnosis')
                if diagnosis_to_remove:
                    patient_diagnosis = PatientDiagnosis.query.filter_by(
                        patient_id=patient.id,
                        icd10_id=int(diagnosis_to_remove)
                    ).first()
                    if patient_diagnosis:
                        db.session.delete(patient_diagnosis)

            # Add new diagnoses if selected

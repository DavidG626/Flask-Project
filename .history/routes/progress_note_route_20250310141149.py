...
def create_progress_note(patient_id):
    ...
    if request.method == 'POST':
        ...
        if has_cpt_codes and 'rfa_other_info' in request.form:
            ...
            # Create RFA items - one per CPT code
            for cpt in selected_cpt_codes:
                # Use the first diagnosis to satisfy the required fields
                # We don't need to combine anything, just pick one
                first_diag = diagnoses_details[0] if diagnoses_details else None
                
                rfa_item = RFAItem(
                    rfa_id=rfa.id,
                    diagnosis=first_diag.description if first_diag else "",
                    icd_code=first_diag.code if first_diag else "",

...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Add to database CHECK THIS! PROBLEM WILL ARISE HERE!
        try:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if provider and check_password_hash(provider.password, form.password.data):
            # Successful login - store username in session and redirect to dashboard
            session['username'] = provider.username  # Store the username in session

...
from models.patient_medical_hx_models import Allergy, PastMedicalHistory, MedicalCondition, Surgery, Medication, LabResult, ImagingResult, OperativeReport, Auth
...
@dashboard_bp.route('/patient/<int:patient_id>/imaging')
@login_required
@check_session_timeout
def patient_imaging(patient_id):
    
    #display patient's imaging results
    patient = Patient.query.get_or_404(patient_id)
...
@dashboard_bp.route('/upload_imaging/<int:patient_id>', methods=['GET', 'POST'])
@login_required
@check_session_timeout
...
@dashboard_bp.route('/view_imaging/<int:imaging_id>')
@login_required
@check_session_timeout
...
@dashboard_bp.route('/download_imaging/<int:imaging_id>')
@login_required
@check_session_timeout
...
@dashboard_bp.route('/delete_imaging/<int:imaging_id>', methods=['POST'])
@login_required
@check_session_timeout
...
# Start of Auths
@dashboard_bp.route('/view_auth/<int:auth_id>')  # Changed from patient_id
@login_required
@check_session_timeout
def view_auth(auth_id):  # Changed from patient_id
    # Get auth record using auth_id, not patient_id
    auth = Auth.query.get_or_404(auth_id)
    
    patient = Patient.query.get(auth.patient_id)
    
    # Fix error message (changed from "imaging" to "authorization")
    if (patient.provider_first_name != current_user.first_name or 
        patient.provider_last_name != current_user.last_name):
        flash('You do not have permission to view this authorization.', 'error')
        return redirect(url_for('dashboard.my_patients'))
    
    return render_template('auth.html', patient=patient)

@dashboard_bp.route('/upload_auth/<int:patient_id>', methods=['GET', 'POST'])  # Changed from auth_id
@login_required
@check_session_timeout
def upload_auth(patient_id):  # Changed from auth_id
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            return redirect(url_for('dashboard.view_auth', patient_id=patient_id)) 
    
    return render_template('auth.html', patient=patient)  

@dashboard_bp.route('/view_auth/<int:auth_id>')
@login_required
@check_session_timeout
...
@dashboard_bp.route('/download_auth/<int:auth_id>')
@login_required
@check_session_timeout
def download_auth(imaging_id):
   
    #download a specific imaging results
    authorization = Auth.query.get_or_404(imaging_id)
...
@dashboard_bp.route('/delete_auth/<int:auth_id>', methods=['POST'])
@login_required
@check_session_timeout
def delete_auth(auth_id):
    # Get the auth result
    authoriztion = Auth.query.get_or_404(auth_id)
    
    # Get the patient to check permissions
    patient = Patient.query.get(authoriztion.patient_id)
    
    # Verify this patient belongs to the current provider
    if (patient.provider_first_name != current_user.first_name or 
        patient.provider_last_name != current_user.last_name):
        flash('You do not have permission to delete this Autho.', 'error')
        return redirect(url_for('dashboard.my_patients'))

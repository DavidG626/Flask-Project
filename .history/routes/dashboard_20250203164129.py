...
def upload_auth(patient_id):  # Changed from auth_id
    ...
    if request.method == 'POST':
        ...
        # Remove duplicate check - you have this twice
        if file.filename == '':
            flash('No file selected', 'error')
            return redirect(request.url)
        ...
        if file:
            ...
            # Create auth record - using patient_id
            authorization = Auth(
                patient_id=patient_id,  # Use patient_id here
                file_name=filename,
                description=request.form.get('description'),
                file_data=file_data,
                file_type=file.content_type
            ...
            return redirect(url_for('dashboard.patient_authorizations', patient_id=patient_id))  # Adjust route name
    
    return render_template('upload_auth.html', patient=patient)  # Changed template name to be consistent
...
@dashboard_bp.route('/download_imaging/<int:patient_id>')
@login_required
@check_session_timeout
def download_imaging(imaging_id):

...
@dashboard_bp.route('/patient/search')
@login_required
def search_patients():
    """
    Search patients by name or claim number
    """
    query = request.args.get('query', '').strip()
    if query:
        patients = Patient.query.filter(
            db.or_(
                Patient.patient_first_name.ilike(f'%{query}%'),
                Patient.patient_last_name.ilike(f'%{query}%'),
                Patient.patient_claim_number.ilike(f'%{query}%')
            ),
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify([{
            'id': p.id,
            'name': f'{p.patient_first_name} {p.patient_last_name}',
            'claim_number': p.patient_claim_number,
            'date_of_injury': p.patient_date_of_injury.strftime('%m/%d/%Y')
        } for p in patients])

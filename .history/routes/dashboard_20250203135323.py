...
@dashboard_bp.route('/delete_imaging_report/<int:report_id>', methods=['POST'])
@login_required
@check_session_timeout
def delete_imaging_report(report_id):
    # Get the report
    report = ImagingResult.query.get_or_404(report_id)
    ...
    try:
        ...
        db.session.commit()
        flash('Imaging report deleted successfully.', 'success')
    except Exception as e:
        db.session.rollback()
        flash('Error deleting report.', 'error')
    
    return redirect(url_for('dashboard.view_lab', patient_id=patient.id))

...
# Edit an ICD10 code
@icd10_bp.route('/edit_icd10/<int:code_id>', methods=['GET', 'POST'])
@login_required
def edit_icd10(code_id):
    code = ICD10Code.query.get_or_404(code_id)
    ...
    if request.method == 'POST':
        ...
        try:
            db.session.commit()
            flash('ICD10 code updated successfully', 'success')
        except:
            db.session.rollback()
            flash('Error updating ICD10 code', 'error')
        
        return redirect(url_for('icd10.icd10_codes'))
    
    return render_template('edit_icd10.html', code=code)
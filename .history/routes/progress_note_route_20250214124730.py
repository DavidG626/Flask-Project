...
def edit_progress_note(note_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Handle diagnoses
            kept_diagnoses = request.form.getlist('keep_diagnoses')
            new_diagnosis = request.form.get('new_diagnosis')

            if kept_diagnoses:
                # Update the patient's diagnoses to match the kept list
                for diagnosis_id in kept_diagnoses:

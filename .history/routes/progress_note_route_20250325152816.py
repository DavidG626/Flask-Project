...
progress_note_bp = Blueprint('progress_note_bp', __name__)
...
def view_progress_note(note_id):
    ...
    if (patient.provider_first_name != current_user.first_name or 
        ...
        return redirect(url_for('pr2.view_rfa', rfa_id=rfa.id))
    
    # Get CPT codes for this progress note
    cpt_relations = ProgressNoteCPT.query.filter_by(progress_note_id=note_id).all()
    cpt_codes = []
    for relation in cpt_relations:
        cpt_codes.append(relation.cpt_code)

    # Get diagnoses if needed (assuming this is relevant for progress notes too)
    diagnoses = []
    if hasattr(progress_note, 'diagnoses') and progress_note.diagnoses:
        # Implement logic to handle diagnoses based on your data model
        pass
...
def delete_progress_note(note_id):
    ...
    try:
        # Delete related CPT codes (if any)
        ProgressNoteCPT.query.filter_by(progress_note_id=progress_note.id).delete()
        
        # Find and delete related RFAs and their items (if any)
        rfas = RequestForAuthorization.query.filter_by(progress_note_id=progress_note.id).all()
        for rfa in rfas:
            RFAItem.query.filter_by(rfa_id=rfa.id).delete()
            db.session.delete(rfa)
        
        # Then delete the progress note
        db.session.delete(progress_note)
        db.session.commit()
        flash('Progress note deleted successfully.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error deleting progress note: {str(e)}', 'error')

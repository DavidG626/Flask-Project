...
from models.patient_medical_hx_models import Allergy, PastMedicalHistory, MedicalCondition, Surgery, Medication, LabResult, ImagingResult, OperativeReport
...
def view_imaging(imaging_id):
    ...
    imaging_result = ImagingResult.query.get_or_404(imaging_id)
    patient = Patient.query.get(imaging_result.patient_id)
    ...
    return send_file(
        io.BytesIO(imaging_result.file_data),
        mimetype=imaging_result.file_type,
        as_attachment=False,
        download_name=imaging_result.file_name
    )
...
@login_required
@check_session_timeout
def download_operative_report(patient_id):
...
@dashboard_bp.route('/upload_i/<int:patient_id>', methods=['GET', 'POST'])
@login_required
@check_session_timeout
...
@login_required
@check_session_timeout
def view_operative_reports(imaging_id):
...
@login_required
@check_session_timeout
def download_operative(imaging_id):
    
    #download a specific operative report result
    operative_reports = OperativeReport.query.get_or_404(imaging_id)
    patient = Patient.query.get(operative_reports.patient_id)
    ...
    # Send file as attachment for download
    return send_file(
        operative_reports.file_path,
        as_attachment=True,
        download_name=operative_reports.file_name
    )

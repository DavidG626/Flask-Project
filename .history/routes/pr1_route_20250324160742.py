    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if has_cpt_codes and 'rfa_other_info' in request.form:
                ...
                return redirect(url_for('pr1_bp.view_rfa', rfa_id=rfa.id))

            flash('PR1 Report created successfully!', 'success')
            return redirect(url_for('pr1.progress_notes', patient_id=patient_id))
...
def view_pr1_rfa(rfa_id):
    ...
    # Get CPT codes for this PR1 report
    pr1_cpt_relations = PR1CPT.query.filter_by(pr1_id=pr1_report.id).all()
    cpt_codes = []
    for relation in pr1_cpt_relations:
        cpt_codes.append(relation.cpt_code)
    ...
    # Simply reuse the view_pr1 template with the RFA data
    return render_template('view_pr1.html', 
                          pr1_report=pr1_report, 
                          patient=patient, 
                          provider=current_user,
                          cpt_codes=cpt_codes,
                          rfa=rfa,  # This will trigger the RFA section in the template
                          rfa_items=rfa_items)

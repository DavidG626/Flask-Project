...
from models.wc_patient_models import Patient, Employer, ClaimsAdmin, Lawyer, PatientDiagnosis
...
def update_medical_history(patient_id):
   ...
   if request.method == 'POST':
       try:
           ...
           medical_history.medical_conditions.clear()
           for i in range(1, 7):
                condition_name = request.form.get(f'medical_condition_{i}')
    ...
    if form.validate_on_submit():
        try:
            ...
            patient = Patient(
                ...
                patient_occupation=form.patient_occupation.data,
            ...
            medical_conditions = [
                ...
                form.medical_condition_2.data,
                form.medical_condition_3.data,
                form.medical_condition_4.data,
                form.medical_condition_5.data,
                form.medical_condition_6.data
            ]
            for condition in medical_conditions:
                if condition:  # Only add if condition was entered
                    medical_condition = MedicalCondition(
                        medical_history_id=medical_history.id,  # Changed from patient_id
...
def update_patient(patient_id):
    ...
    if request.method == 'POST':
        try:
            ...
            patient.patient_phone = request.form.get('patient_phone')
            patient.patient_occupation = request.form.get('patient_occupation')
            patient.patient_sex = request.form.get('patient_sex')
            ...
            # Only update diagnoses if new ones were selected
            selected_diagnoses = request.form.getlist('diagnoses')
            if selected_diagnoses:
                # Remove patient's old diagnosis links (not the ICD10 codes themselves)
                PatientDiagnosis.query.filter_by(patient_id=patient.id).delete()

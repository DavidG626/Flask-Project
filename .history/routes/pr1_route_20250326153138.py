    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if has_cpt_codes and 'rfa_other_info' in request.form:
                ...
                return redirect(url_for('pr1_bp.view_pr1_rfa', rfa_id=rfa.id))
...
#view RFA
@pr1_bp.route('/rfa/<int:rfa_id>')
@login_required
@check_session_timeout
def view_pr1_rfa(rfa_id):
    # Get the RFA
    rfa = PR1Authorization.query.get_or_404(rfa_id)
    
    # Get the associated PR1 report
    pr1_report = PR1Report.query.get_or_404(rfa.pr1_id)
    
    # Check permissions (optional, since view_pr1 will also check)
    patient = Patient.query.get(pr1_report.patient_id)
    if (patient.provider_first_name != current_user.first_name or 
        patient.provider_last_name != current_user.last_name):
        flash('You do not have permission to view this report.', 'error')
    ...
    # Redirect to the PR1 view that will display both the PR1 and its RFA
    return redirect(url_for('pr1_bp.view_pr1', pr1_id=pr1_report.id))
...
def view_pr1_rfa(rfa_id):
    ...
    pr1_report = PR1Report.query.get(rfa.pr1_id)
    ...
    return render_template('view_pr1_rfa.html', rfa=rfa, patient=patient, pr1_report=pr1_report, rfa_items=rfa_items, provider=current_user)

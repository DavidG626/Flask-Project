...
def update_medical_history(patient_id):
    # Replace get_or_404 with get() and abort if not found
    ...
    for diag in patient_diagnoses:
        icd10_code = db.session.get(ICD10Code, diag.icd10_id)
        if icd10_code:
    ...
    if search_term:
        ...
        ).order_by(ICD10Code.description).limit(10).all()
    else:
        icd10_codes = []

    print("ICD10 codes found:", icd10_codes)  # Add this here
    ...
    if request.method == 'POST':
        try:
            ...
            # First, clear existing diagnoses
        ...
        print("ICD10 codes found:", icd10_codes)

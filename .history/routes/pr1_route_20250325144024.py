...
def delete_pr1(note_id):
    ...
    if (patient.provider_first_name != current_user.first_name or 
        patient.provider_last_name != current_user.last_name):
        flash('You do not have permission to delete this PR!.', 'error')
        return redirect(url_for('dashboard.my_patients'))
    
    try:
        db.session.delete(pr1_bp)
        db.session.commit()
        flash('PR1 deleted successfully.', 'success')
    except Exception as e:
        db.session.rollback()
        flash('Error deleting pr1 note.', 'error')

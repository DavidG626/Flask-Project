...
def cpt_codes():
    ...
    if search_query:
        cpt_codes = CPTCode.query.filter(
            db.or_(
                CPTCode.code.ilike(f'%{search_query}%'),
                CPTCode.description.ilike(f'%{search_query}%'),
                CPTCode.category.ilike(f'%{search_query}%')
            )
        ).order_by(CPTCode.code).all()
    else:
        cpt_codes = CPTCode.query.order_by(CPTCode.code).all()
    
    return render_template('cpt_codes.html', cpt_codes=cpt_codes, search_query=search_query)
